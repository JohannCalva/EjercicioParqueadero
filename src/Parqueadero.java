...
// -----------------------------------------------------------------
    ...
    {   int carrosPB = contarCarrosQueComienzanConPlacaPB();
        if(hayCarroCon24Horas()){
            return "Cantidad de carros con placa PB: " + carrosPB + "- Hay carro parqueado por 24 o mas horas: Si";
        }else{
            return "Cantidad de carros con placa PB: " + carrosPB + "- Hay carro parqueado por 24 o mas horas: No";
        }
    }
    ...
    public double darTiempoPromedio(){
        ...
        for(Puesto puesto : puestos){
            if(puesto.estaOcupado()){
                contadorCarros++;
                sumaTiempos += puesto.darCarro().darTiempoEnParqueadero(horaActual);
            }
        }
        if(contadorCarros == 0){
            return 0;
        ...
        tiempoPromedio = sumaTiempos/contadorCarros;
        return tiempoPromedio;
    }
    public Carro retornaCarroMayorTiempo(){
        Carro carroMayorTiempo = null;
        Carro carroActual;
        for(Puesto puesto : puestos){
            if(puesto.estaOcupado()){
                carroActual = puesto.darCarro();
    ...
    public boolean hayCarrosPlacaIgual(){
        ...
        for(Puesto puesto : puestos){
            if(puesto.estaOcupado()){
                placaActual = puesto.darCarro().darPlaca();
                for(int i = 0; i < TAMANO; i++){
                    if(puestos[i].estaOcupado()){
                        if(puestos[i].darCarro().darPlaca().equals(placaActual)){
                            return true;
                        }
                    }
                }
